...
export function ActivitiesPage() {
 ...
 return <><Hero image={photo('activiteiten-hero-wijngaard',1920)} srcSet={`${photo('activiteiten-hero-wijngaard',960)} 960w, ${photo('activiteiten-hero-wijngaard',1920)} 1920w, ${photo('activiteiten-hero-wijngaard',2048)} 2048w`} title={t('activities.heroTitle')} body={t('activities.heroBody')} alt={t('activities.heroAlt')} className="activities-hero">{[['/reserveren','nav.book','purple'],['/pluspunten','nav.benefits','green']].map(([path,key,color]) => <a key={path} className={`button button--${color}`} href={siteUrl(path)} onClick={e => {e.preventDefault();navigate(path);}}>{t(key)}</a>)}</Hero>

...
export function ActivitiesPage() {
 ...
 return <><Hero image={photo('activiteiten-hero',1920)} srcSet={`${photo('activiteiten-hero',960)} 960w, ${photo('activiteiten-hero',1920)} 1920w, ${photo('activiteiten-hero',2560)} 2560w`} title={t('activities.heroTitle')} body={t('activities.heroBody')} alt={t('activities.heroAlt')} className="activities-hero">{[['/reserveren','nav.book','purple'],['/pluspunten','nav.benefits','green']].map(([path,key,color]) => <a key={path} className={`button button--${color}`} href={path} onClick={e => {e.preventDefault();navigate(path);}}>{t(key)}</a>)}</Hero>
